 ...
 */
uint16_t stscFlags = 0;
uint16_t stscStatusFlags = 0;
float stsc_w_z_baro_p = 0.35f;
float stsc_w_z_sonar_p = 0.7f;
float stsc_sonarBaro = 0.5;
float stsc_baroAccCorr = 0.0f;
float stsc_BaroSonarCorr = 0.0f;
//int16_t getPosEstimatorBaroAlt() { return posEstimator.baro.alt; }
int16_t getBaroAltitudeOffset() { return (int16_t) posEstimator.baro.offset; }
//int32_t getAccelBiasZ() { return (int32_t) (posEstimator.est.epv*100.0); }
float stsc_corr = 0;
int32_t getAccelBiasZ() { return (int32_t) (stsc_corr*1000); }

float baroResidual = 0;
float sonarResidual = 0;
float sonarBaroResidual = 0;

int16_t getRawBaroAlt() { return posEstimator.baro.alt; }
int16_t getRawBaroAltMean() { return posEstimator.baro.altMean; }
int16_t getPosEstimatorVelZ() { return posEstimator.est.vel.V.Z; }
int16_t getPosEstimatorPosZ() { return posEstimator.est.pos.V.Z; }
//int16_t getBaroResidual() { return baroResidual; }
//int16_t getSonarResidual() { return sonarResidual; }
int16_t getBaroResidual() { return baroResidual; }
int16_t getSonarBaroResidual() { return sonarBaroResidual; }

int16_t getAccelZ() { return posEstimator.imu.accelNEU.V.Z * 100; }


bool stscSimulateArmed() { return (stscFlags&1)>0; }
bool stscUseSonar() { return (stscFlags&2)>0; }
bool stscUseBaro() { return (stscFlags&4)>0; }
bool stscUseAccelBias() { return (stscFlags&8)>0; }


int16_t getPosEstimatorBaroAlt()		{ return posEstimator.baro.alt; }
int16_t getPosEstimatorBaroAltMean()	{ return posEstimator.baro.altMean; }
int16_t getPosEstimatorBaroVel()		{ return baroResidual; }

float stsc_baroSmooth = 0.6f;
...
static void updateBaroTopic(timeUs_t currentTimeUs)
{
    ...
    if (updateTimer(&baroUpdateTimer, HZ2US(INAV_BARO_UPDATE_RATE), currentTimeUs))
    {
        ...
        posEstimator.baro.rawAltMean += 0.05f * (posEstimator.baro.rawAlt - posEstimator.baro.rawAltMean);

        /* If we were never armed - keep altitude at zero */
        if ((!ARMING_FLAG(ARMED) && !stscSimulateArmed()) && !ARMING_FLAG(WAS_EVER_ARMED))
        {
        	//posEstimator.baro.offset += 0.01f * (baroAlt - posEstimator.baro.offset);
        	posEstimator.baro.offset = posEstimator.baro.rawAltMean;
        	posEstimator.baro.altMean = 0;
        }

        if (sensors(SENSOR_BARO) && isBaroCalibrationComplete())
        {
            float baroDt = (currentTimeUs - posEstimator.baro.lastUpdateTime) * 1e-6;
            float newAlt = posEstimator.baro.rawAlt - posEstimator.baro.offset;

            /*
            float deltaAlt = newAlt - posEstimator.baro.alt;
            float vel = 0;
            if(baroDt > 0.0)
            {
            	vel = deltaAlt / baroDt;
            ...
            posEstimator.baro.vel += 0.01f * (vel - posEstimator.baro.vel);
*/
            posEstimator.baro.alt = newAlt;
            posEstimator.baro.epv = positionEstimationConfig()->baro_epv;

            posEstimator.baro.lastUpdateTime = currentTimeUs;


			float baroDelta = posEstimator.baro.alt - posEstimator.baro.altMean;

			float vel = posEstimator.est.vel.V.Z;
			if(vel < 0 ) vel = -vel;
			if(vel > 100) vel = 100;

			float fac = stsc_baroSmooth / 100.0 * vel;

			if((baroDelta * posEstimator.est.vel.V.Z) < 0)
				fac /= 2;

			fac += 0.02f;

            posEstimator.baro.altMean += fac * baroDelta;


            /*
            if(posEstimator.sonar.altMean>10 && posEstimator.sonar.altMean<200)
            {
//				sonarBaroResidual = posEstimator.sonar.altMean - (posEstimator.baro.rawAltMean - posEstimator.baro.offset);
				sonarBaroResidual = posEstimator.sonar.altMean - posEstimator.baro.altMean;
				sonarBaroResidual *= stsc_BaroSonarCorr;

				posEstimator.baro.offset -= sonarBaroResidual;
            }
            else
            {
            	sonarBaroResidual = 0;
...
int16_t getPosEstimatorSonarAlt() 		{ return posEstimator.sonar.alt; }
int16_t getPosEstimatorSonarAltMean() 	{ return posEstimator.sonar.altMean; }
int16_t getPosEstimatorSonarVel() 		{ return posEstimator.sonar.vel * 100.0f; }
    ...
    if (updateTimer(&sonarUpdateTimer, HZ2US(INAV_SONAR_UPDATE_RATE), currentTimeUs)) {
        if (sensors(SENSOR_SONAR)) {
            ...
            if (newSonarAlt > 0 && newSonarAlt <= INAV_SONAR_MAX_DISTANCE)
            {
                ...
                posEstimator.sonar.altMean += 0.10f * (posEstimator.sonar.alt - posEstimator.sonar.altMean);
                return;
            }
        }
		...
		posEstimator.sonar.altMean = 0;
		posEstimator.sonar.meanCount = 0;
    }
}
#endif
...
static void updateIMUTopic(void)
{
    ...
    if (!isImuReady()) {
        ...
        if ((!ARMING_FLAG(ARMED) && !stscSimulateArmed()) && positionEstimationConfig()->accz_unarmed_cal) {
...
static void stscSetFlag(uint16_t val, bool valid)
{
	if(valid)
		stscStatusFlags |= val;
	else
		stscStatusFlags &= ~((uint16_t)val);

}


uint16_t stsc_mode = 0;
float stsc_baro_wz = 0.6f;// 0.35f;

static void updateEstimatedTopic3(timeUs_t currentTimeUs)
{
    ...
    stscSetFlag(1, isGPSValid);
    stscSetFlag(2, isBaroValid);
    stscSetFlag(4, isSonarValid);

    /* Do some preparations to data */
    if (isBaroValid) {
        if (!ARMING_FLAG(ARMED) && !stscSimulateArmed()) {
    ...
    bool isAirCushionEffectDetected = false; /*ARMING_FLAG(ARMED) &&
...
//    float baroResidual;
    ...
    if (isBaroValid) {
    	...
    	if(stsc_mode==1)
    		...
    		baroResidual = (isAirCushionEffectDetected ? posEstimator.state.baroGroundAlt : posEstimator.baro.altMean) - posEstimator.est.pos.V.Z;
    	else
    		baroResidual = 0;
    }
    ...
    if (isEstZValid || useGpsZPos || isBaroValid) {
        ...
        if (isBaroValid) {
            ...
            inavFilterCorrectPos(Z, dt, baroResidual, stsc_baro_wz);//positionEstimationConfig()->w_z_baro_p);

            /* Adjust EPV */
            newEPV = updateEPE(posEstimator.est.epv, dt, posEstimator.baro.epv, stsc_baro_wz);//positionEstimationConfig()->w_z_baro_p);
        }
#endif
    ...
    if (!ARMING_FLAG(ARMED) && !stscSimulateArmed())
    {
        posEstimator.est.pos.A[Z] = 0;
        posEstimator.est.vel.A[Z] = 0;
    }
}
...
void initializePositionEstimator(void)
{
    ...
    posEstimator.gps.lastUpdateTime = 0;
    posEstimator.baro.lastUpdateTime = 0;
    posEstimator.baro.offset = 0;
    posEstimator.baro.rawAltMean = 0;
    posEstimator.baro.rawAlt = 0;
    posEstimator.baro.altMean = 0;
...
void updatePositionEstimator(void)
{
    ...
    /* Update estimate */
    updateEstimatedTopic3(currentTimeUs);
...
/*
static void updateEstimatedTopic2(timeUs_t currentTimeUs)
{
    if (!isImuReady()) {
        return;
    }
 ...
 //   posEstimator.est.vel.A[Z] *= 0.99;
    ...
    posControl.navConfig->estimation.w_z_baro_p = stsc_w_z_baro_p;


    baroResidual = posEstimator.baro.altMean - posEstimator.est.pos.V.Z;
    ...
    if (posControl.navConfig->estimation.w_acc_bias > 0)
    {
    	...
    	accelBiasCorr.V.Z -= 1.0 * baroResidual * sq(posControl.navConfig->estimation.w_z_baro_p);
