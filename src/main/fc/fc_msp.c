...
// ++++ STSC
extern uint16_t stsc_statusFlags;
extern uint16_t stsc_flags;
extern float 	stsc_baroVelScale;
extern float 	stsc_baroVelScaleMin;
extern uint16_t stsc_mode;
extern float 	stsc_baro_wz;
extern float stsc_fake_sonar;
extern float stsc_sonar_wz;


int32_t rangefinderGetLatestAltitude();

int16_t getRawBaroAlt();
int16_t getRawBaroAltMean();
int16_t getBaroOffset();
int16_t getBaroResidual();
int16_t getSonarResidual();
int16_t getSonarBaroResidual();

int16_t getAccelZ();
int16_t getPosEstimatorPosZ();
int16_t getPosEstimatorVelZ();
int16_t getPosEstimatorBaroAlt();
int16_t getPosEstimatorBaroAltMean();
int16_t getPosEstimatorSonarAlt();
int16_t getPosEstimatorSonarAltMean();
...
static bool mspFcProcessOutCommand(uint8_t cmdMSP, sbuf_t *dst, mspPostProcessFnPtr *mspPostProcessFn)
{
	...
	case MSP_STSC_SONAR:
        sbufWriteU16(dst, stsc_statusFlags);
		sbufWriteU16(dst, getAccelZ());
        sbufWriteU16(dst, getPosEstimatorPosZ());
        sbufWriteU16(dst, getPosEstimatorVelZ());
		sbufWriteU16(dst, getPosEstimatorSonarAlt());
        sbufWriteU16(dst, getPosEstimatorSonarAltMean());
        sbufWriteU16(dst, getPosEstimatorBaroAlt());
        sbufWriteU16(dst, getPosEstimatorBaroAltMean());
        sbufWriteU16(dst, getBaroResidual());
        sbufWriteU16(dst, getSonarResidual());
        sbufWriteU16(dst, getSonarBaroResidual());
        sbufWriteU16(dst, getBaroOffset());
//        sbufWriteU16(dst, getPosEstimatorBaroVel());
//        sbufWriteU16(dst, getPosEstimatorSonarVel());

		/*
        sbufWriteU16(dst, (int16_t)getRawBaroAlt());
        sbufWriteU16(dst, (int16_t)getRawBaroAltMean());
		sbufWriteU16(dst, getPosEstimatorSonarAlt());
        sbufWriteU16(dst, getPosEstimatorSonarVel());
        sbufWriteU16(dst, (int16_t)getPosEstimatorVelZ());
        sbufWriteU16(dst, (int16_t)getPosEstimatorPosZ());
        sbufWriteU16(dst, (int16_t)getPosEstimatorBaroAlt());
        sbufWriteU16(dst, getBaroAltitudeOffset());
        sbufWriteU32(dst, getAccelBiasZ());
        sbufWriteU16(dst, getBaroResidual());
        sbufWriteU16(dst, getSonarBaroResidual());
        sbufWriteU16(dst, getBaroTemp());
        sbufWriteU16(dst, getPosEstimatorSonarAltMean());
        sbufWriteU16(dst, stscStatusFlags);
*/
	...
	case MSP_STSC_BARO:
//        sbufWriteU32(dst, getBaroRawPressure());
//        sbufWriteU32(dst, getBaroRawTemp());
		break;
		
	case MSP_STSC_POS_EST:
		break;
...
static mspResult_e mspFcProcessInCommand(uint8_t cmdMSP, sbuf_t *src)
{
    ...
    case MSP_STSC_SETFLAG:
    	stsc_flags = sbufReadU16(src);
    	break;

    case MSP_STSC_SETSONAR:
    	stsc_mode = (sbufReadU16(src));
    	stsc_baroVelScale = ((float)sbufReadU16(src) / 100.0f);
    	stsc_baroVelScaleMin = ((float)sbufReadU16(src) / 100.0f);
    	stsc_baro_wz = ((float)sbufReadU16(src) / 100.0f);
    	stsc_fake_sonar = ((float)sbufReadU16(src) );
    	stsc_sonar_wz = ((float)sbufReadU16(src) / 100.0f);
