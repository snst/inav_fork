    ...
    //            , { 0 }
     ...
     //       if(Math.Abs(accF) < 20)
       //         _v = 0;
            ...
            double lastLastBaroAlt = lastBaroAlt;

            lastBaroAlt += (double)0.1 * (median - lastBaroAlt);

            double newBaroVel = (lastBaroAlt - lastLastBaroAlt) / dt;

            _baroVel += 0.05 * (newBaroVel - _baroVel);

            this.chart.Series[0].Points.AddXY(ts, baroAlt);
            this.chart.Series[1].Points.AddXY(ts, median);
            this.chart.Series[2].Points.AddXY(ts, lastBaroAlt);
            this.chart.Series[3].Points.AddXY(ts, accF / 50);
            this.chart.Series[4].Points.AddXY(ts, _v);
            this.chart.Series[5].Points.AddXY(ts, _s);
            this.chart.Series[6].Points.AddXY(ts, _baroVel);
