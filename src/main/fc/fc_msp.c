...
int16_t getPosEstimatorSonarVel();
...
static bool mspFcProcessOutCommand(uint8_t cmdMSP, sbuf_t *dst, mspPostProcessFnPtr *mspPostProcessFn)
{
	...
	case MSP_STSC_SONAR:
		sbufWriteU16(dst, (int16_t) rangefinderGetLatestAltitude());
		sbufWriteU16(dst, getPosEstimatorSonarAlt());
        sbufWriteU16(dst, getPosEstimatorSonarVel());
        sbufWriteU16(dst, (int16_t)baroCalculateAltitude());
