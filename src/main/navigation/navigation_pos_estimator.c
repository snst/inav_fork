...
uint16_t stsc_statusFlags = 0;
uint16_t stsc_flags = 0;
float 	 stsc_baroVelScale = 0.65;
float    stsc_baroVelScaleMin = 0.1f;
uint16_t stsc_mode = 1;
float 	 stsc_baro_wz = 0.35;
float    stsc_sonar_wz = 0.01;

bool stscSimulateArmed() { return (stsc_flags&1)>0; }

bool stscUseSonar()      { return (stsc_flags&2)>0; }
bool stscUseBaro()       { return (stsc_flags&4)>0; }
bool stscUseAccelBias()  { return (stsc_flags&8)>0; }

float baroResidual = 0;
float sonarResidual = 0;
float sonarBaroResidual = 0;
float stsc_fake_sonar = 0;

int16_t getRawBaroAlt()                 { return posEstimator.baro.alt; }
int16_t getRawBaroAltMean()             { return posEstimator.baro.altMean; }
int16_t getBaroOffset()                 { return posEstimator.baro.offset2; }
...
int16_t getPosEstimatorBaroAlt()	    { return posEstimator.baro.alt + posEstimator.baro.offset2; }
int16_t getPosEstimatorBaroAltMean()	{ return posEstimator.baro.altMean + posEstimator.baro.offset2; }
...
static void updateBaroTopic(timeUs_t currentTimeUs)
{
    ...
    if (updateTimer(&baroUpdateTimer, HZ2US(INAV_BARO_UPDATE_RATE), currentTimeUs)) 
    {
        ...
        {
        	posEstimator.baro.alt = posEstimator.baro.rawAlt - posEstimator.baro.offset;
			...
			float fac = stsc_baroVelScale / 100.0 * vel;

			if((baroDelta * posEstimator.est.vel.V.Z) < 0)
				fac /= 2;

			fac += stsc_baroVelScaleMin;
			if(fac>1)
				fac = 1;
...
static void updateEstimatedTopic(timeUs_t currentTimeUs)
{
    ...
    float w_z_baro_p = stsc_baro_wz; //positionEstimationConfig()->w_z_baro_p;
    float baroAlt = posEstimator.baro.alt;

    if(stsc_mode==1)
    	baroAlt = posEstimator.baro.altMean;



    float sonarAlt = (stsc_fake_sonar>0) ? stsc_fake_sonar : posEstimator.sonar.alt;

    if ((!ARMING_FLAG(ARMED) && !stscSimulateArmed()))
	{
    	posEstimator.baro.offset2 = 0;
	}
    else
    {
        if(sonarAlt>15)
        {
        	sonarResidual = sonarAlt - posEstimator.est.pos.V.Z;
        	sonarBaroResidual = sonarAlt - baroAlt;
        	posEstimator.baro.offset2 += stsc_sonar_wz * (sonarBaroResidual - posEstimator.baro.offset2);
        }
        else
        {
        	sonarResidual = 0;
        ...
        baroAlt += posEstimator.baro.offset2;
    }
 ...
 //                                        (isBaroValid && posEstimator.state.isBaroGroundValid && baroAlt < posEstimator.state.baroGroundAlt));
...
//    float baroResidual;
    ...
    if (isBaroValid) {
        ...
        baroResidual = (isAirCushionEffectDetected ? posEstimator.state.baroGroundAlt : baroAlt) - posEstimator.est.pos.V.Z;
    }
    ...
    if (isEstZValid || useGpsZPos || isBaroValid) {
        ...
        if (isBaroValid) {
            ...
            inavFilterCorrectPos(Z, dt, baroResidual, w_z_baro_p);

            /* Adjust EPV */
            newEPV = updateEPE(posEstimator.est.epv, dt, posEstimator.baro.epv, w_z_baro_p);
        }
#endif
    ...
    if (positionEstimationConfig()->w_acc_bias > 0) {
        ...
        if (isBaroValid && !isAirCushionEffectDetected) {
            accelBiasCorr.V.Z -= baroResidual * sq(w_z_baro_p);
        }
    ...
    if(posEstimator.est.pos.V.Z<-5)
    {
    	posEstimator.est.pos.V.Z = 0;
    	posEstimator.est.vel.V.Z = 0;
